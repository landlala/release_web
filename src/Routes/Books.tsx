...
function Books() {
  ...
  function smallBookScreen() {
    const currentPosting = (posts: any, contents: number) => {
      ...
      currentPosts = posts.slice(
        indexOfFirst + contents * 2,
        indexOfFirst + contents * 2 + 2
      );
      return currentPosts;
    };
    return (
  ...
  function largeBookScreen() {
    const currentPosting = (posts: any, contents: number) => {
      let currentPosts = [];
      currentPosts = posts.slice(
        indexOfFirst + contents * 4,
        indexOfFirst + contents * 4 + 4
      );
      return currentPosts;
    };
    return (
      ...
      <Nav>
        ...
        <StyledLink to="/books">
          <motion.img
            onClick={moveToTop}
            style={{ opacity: 1 }}
            src="/release_web/img/books2.png"
          />
        </StyledLink>
        <StyledLink to="/login">
          <motion.img
